          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400 mt-0.5">
                Week {current.week} · {formatDate(current.date)}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          <div className="rounded-xl bg-rose-50 p-2.5 text-center">
            <p className="text-lg font-black text-rose-700">
              {current.appeared - current.passed}
            </p>
            <p className="text-[10px] text-rose-400 font-medium uppercase tracking-wide mt-0.5">
              Failed
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 bg-indigo-50/60 rounded-xl p-3">
          <div className="flex-1">
            ...
            <div className="flex items-center gap-2 mt-1 text-[10px] text-slate-400">
              <span>
                High: <b className="text-slate-600">{current.highestScore}</b>
              </span>
              <span>·</span>
              <span>
                Low: <b className="text-slate-600">{current.lowestScore}</b>
              </span>
              <span>·</span>
              <span>
                Pass: <b className="text-emerald-600">{current.passPercent}%</b>
              </span>
            </div>
          </div>
        </div>
            ...
            {current.scoreDistribution.map((d) => {
              ...
              return (
                <div key={d.range} className="flex items-center gap-2">
                  ...
                  <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                    <div
                      className={cn(
                        ...
                        d.range.startsWith("8")
                          ? "bg-emerald-400"
                          : d.range.startsWith("6")
                            ? "bg-indigo-400"
                            : d.range.startsWith("4")
                              ? "bg-amber-400"
                              : "bg-rose-400",
                      )}
                      style={{ width: `${pct}%` }}
                  ...
                  <span className="text-[10px] font-semibold text-slate-600 w-6 text-right">
                    {d.count}
                  </span>
                </div>
              );
