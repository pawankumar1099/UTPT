...
const submissionSchema = new mongoose.Schema(
  { problem: String, difficulty: { type: String, enum: ['Easy','Medium','Hard'] },
    status: String, language: String, submittedAt: Date,
...
const platformStatSchema = new mongoose.Schema(
  { totalSolved: { type: Number, default: 0 },
    ...
    globalRanking: Number, globalRankingChange: Number,
    acceptanceRate: Number, totalSubmissions: Number,
    contestsParticipated: Number, contestRating: Number,
...
const codingStatSchema = new mongoose.Schema(
  { userId: { type: mongoose.Schema.Types.ObjectId, ref: 'User', required: true, unique: true },
    ...
    problemsOverTime: [{ date: String, leetcode: Number, codeforces: Number, total: Number, _id: false }],
    submissionCalendar: { leetcode: [Number], codeforces: [Number], combined: [Number] },
    lastSyncedAt: Date },
  { timestamps: true }
);
